...
namespace DB
{
        ...
        public bool CreateDatabase(string server, string login, string password, string databaseName, string path)
        {
            ...
            try
            {
                using (SqlConnection dbcon = new SqlConnection(strDBConnection))
                {
                    ...
                    strSqlTx = "CREATE TABLE [dbo].[ShipList]( " +
    ...
    public class DBInterface
    {
        ...
        //출고
        public int Outgoing(string id, string model, string outhandler, string toplant)
        {
            ...
            using (SqlConnection dbcn = new SqlConnection(SQLConnectionInfo))
            {
                try
                {
                    ...
                    cmdSql = new SqlCommand(strSql, dbcn);
                    int noship = Convert.ToInt32(cmdSql.ExecuteScalar());
                    cmdSql.Dispose();
                    if (noship == 1)
                    {
                        dbcn.Close();
                    ...
                    strSql = "UPDATE ShipList SET CurrentCount = COALESCE(CurrentCount, 0) + 1 " +
                            "Model =  '"+ model + "' and [Quantity] <> [CurrentCount] OR [CurrentCount] IS NULL";
                    SqlCommand cmdSql2 = new SqlCommand(strSql, dbcn);
                    cmdSql1.ExecuteNonQuery();
                    cmdSql1.Dispose();
                    LogMsg.CMsg.Show("DBI", "update ship list ", "", false, true);
                ...
                {
                    LogMsg.CMsg.Show("DBI", "outgoing error", e.ToString(), true, true);
        ...
        public List<string> GetList(string col, string condition)
        {
            ...
            using (SqlConnection dbcn = new SqlConnection(SQLConnectionInfo))
            {
                ...
                {
                    LogMsg.CMsg.Show("DBI", "get list error: " + ex.Message, "", true, true);
                    return null;
                }
            }
